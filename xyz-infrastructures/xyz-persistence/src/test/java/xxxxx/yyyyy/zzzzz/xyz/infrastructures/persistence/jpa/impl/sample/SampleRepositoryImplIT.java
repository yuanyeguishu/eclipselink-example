...
//    public TestName name = new TestName();
    ...
    public void UnitOfWork_StateUnderTest_ExpectedBehavior1() { // TODO should change method name
        try {
            this.sampleRepository.findAll().stream().forEach(this.sampleRepository::remove);
    ...
    public void UnitOfWork_StateUnderTest_ExpectedBehavior2() { // TODO should change method name
        try {
            LongStream.range(0L, 10L).boxed().map(x -> new Sample(x, String.valueOf(x))).forEach(this.sampleRepository::store);
            List<Sample> samples = this.sampleRepository.findAll();
